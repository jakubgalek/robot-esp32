...
void setup() {
  ...
  try {
    ...
    showMessageDialog(null, "COM port is not available (may be in use by another program)");
    println("Error:", e);
    exit();
  }
}
...
for (int i = 0; i < 360; i++) {
  ...
  float x = float(640) - (map_values(data[i])) * sin(radians(i));
  float y = float(360) + (map_values(data[i])) * cos(radians(i));
